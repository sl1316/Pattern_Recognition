...
from sklearn.utils.extmath import fast_logdet, fast_dot, randomized_svd, squared_norm
...
class FactorAnalysis(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        #print np.dot(tmp, cov_z)[0]
        X_transformed = fast_dot(tmp, cov_z)
        #print np.asarray(cov_z).shape
       # print X_transformed[0]
        return X_transformed
    def inver_transform(self,X_transformed):
        Ih = np.eye(len(self.components_))
        Wpsi = self.components_ / self.noise_variance_
        cov_z = linalg.inv(Ih + np.dot(Wpsi, self.components_.T))
        X = np.dot(X_transformed,matrix(cov_z).I)
        X = np.dot(X, matrix(Wpsi.T).I)
        X = X + self.mean_
        return X
